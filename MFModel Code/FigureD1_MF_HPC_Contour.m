...
% S_IE is no longer indexed to SII!
function [] = FigureD1_MF_HPC_Contour(S_ElgnInd,S_IlgnInd,rI_L6Ind,...
                                       S_EEInd,S_IIInd)
...
%L6 input S_IEOneTime = 0.204*S_II;
...
%% Major Change HERE:
S_IEtest = linspace(S_IE_Mtp(1)*min(S_IItest),S_IE_Mtp(2)*max(S_IItest),GridNum2);
...
% Change here: All the boundary lines are horizontal now, so doesn't
% affected by the S_IE(x)-axis change
LineL1 = polyfit([0.1  0.2 ],[0.8 0.8],1); % S_IEMtp first, second S_EIMtp. Those numbers are multipliers of S_II and S_EE
LineL2 = polyfit([0.06 0.28],[0.8 0.8],1);
