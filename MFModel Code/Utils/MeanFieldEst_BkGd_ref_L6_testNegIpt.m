...
% Ver 4:       L6 added
% Ver Test5:   Try to represent negative rate input
% iteration
function [f_EnIOut,meanVs,loop,SteadyIndicate] = MeanFieldEst_BkGd_ref_L6_testNegIpt(C_EE,C_EI,C_IE,C_II,... %4
...
%f_EnI0 = abs(f_EnI0);      
%% NOTE: I am not modifying negative firing rates now! I will reflect it in the single cell simulations
%% The new input!
...
%% Incoporate rates if negative:
rE = abs(f_EnI(1)/1000);  rI = abs(f_EnI(2)/1000); % f_EnI in s^-1, but here we use ms^-1
S_E = S_E*sign(f_EnI(1)); S_I = S_I*sign(f_EnI(2));
