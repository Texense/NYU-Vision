...
% Ver Test3.3: Doing Newton for composition seperatly does not work. So Now
% we do it as whole.
% iteration
function [f_EnIOut,meanVs,loop,SteadyIndicate] = MeanFieldEst_BkGd_Newton2_StepSize_TestmV(C_EE,C_EI,C_IE,C_II,... %4
...
del = 1/200; % relative purturbation size
...
mVE = 0.5; mVI = 0.7; 
meanVs = [mVE;mVI];
f_EnIIni = MeanFieldEst_BkGd(N_EE,N_EI,N_IE,N_II,...
...
%% Choose 4 seeds for purturbations    
SeedPool = randi(2^32-1,2,2);
SeedeE = SeedPool(1); SeedeI = SeedPool(2); % seeds for mVE
SeediE = SeedPool(3); SeediI = SeedPool(4); % seeds for mVI
    
%% simulate one neuron with input                             
[mVE,~] = MEanFieldEst_SingleCell_4caSeq('e', f_EnIIni, SeedeE, SeedeI, ...
                                         N_EE,N_EI,N_IE,N_II,...
                                         S_EE,S_EI,S_IE,S_II,p_EEFail,...
                                         lambda_E,S_Elgn,rE_amb,S_amb,...
                                         lambda_I,S_Ilgn,rI_amb,...
...
[mVI,~] = MEanFieldEst_SingleCell_4caSeq('i', f_EnIIni, SeediE, SeediI, ...
                                         N_EE,N_EI,N_IE,N_II,...
                                         S_EE,S_EI,S_IE,S_II,p_EEFail,...
                                         lambda_E,S_Elgn,rE_amb,S_amb,...
                                         lambda_I,S_Ilgn,rI_amb,...
...
f_EnI0 = MeanFieldEst_BkGd(N_EE,N_EI,N_IE,N_II,...
                                   ...
                                   lambda_I,S_Ilgn,rI_amb,...
                                   gL_E,gL_I,Ve,Vi,mVE,mVI);
%% Simulate purturbed firing rates for dfE
dfE = zeros(size(f_EnIIni)); dfE(1) = f_EnIIni(1)/del;
f_EnI_dfE = f_EnIIni+dfE;                           
                               
[mVE_dfE,~] = MEanFieldEst_SingleCell_4caSeq('e', f_EnI_dfE, SeedeE, SeedeI,...
                                         N_EE,N_EI,N_IE,N_II,...
                                         S_EE,S_EI,S_IE,S_II,p_EEFail,...
                                         lambda_E,S_Elgn,rE_amb,S_amb,...
                                         lambda_I,S_Ilgn,rI_amb,...
                                         ...
                                         rhoE_ampa,rhoE_nmda,rhoI_ampa,rhoI_nmda,...
                                         gL_E,gL_I,Ve,Vi,LIFSimuT);
[mVI_dfE,~] = MEanFieldEst_SingleCell_4caSeq('i', f_EnI_dfE, SeediE, SeediI,...
                                         N_EE,N_EI,N_IE,N_II,...
                                         S_EE,S_EI,S_IE,S_II,p_EEFail,...
                                         lambda_E,S_Elgn,rE_amb,S_amb,...
                                         lambda_I,S_Ilgn,rI_amb,...
...
f_EnI0_dfE = MeanFieldEst_BkGd(N_EE,N_EI,N_IE,N_II,...
                                   S_EE,S_EI,S_IE,S_II,p_EEFail,...
                                   lambda_E,S_Elgn,rE_amb,S_amb,...
                                   lambda_I,S_Ilgn,rI_amb,...
                                   gL_E,gL_I,Ve,Vi,mVE_dfE,mVI_dfE);                               
                               
%% Simulate purturbed firing rates for dfI
dfI = zeros(size(f_EnIIni)); dfI(2) = f_EnIIni(2)/del;
f_EnI_dfI = f_EnIIni+dfI;                           
                               
[mVE_dfI,~] = MEanFieldEst_SingleCell_4caSeq('e', f_EnI_dfI, SeedeE, SeedeI,...
                                         N_EE,N_EI,N_IE,N_II,...
                                         S_EE,S_EI,S_IE,S_II,p_EEFail,...
                                         lambda_E,S_Elgn,rE_amb,S_amb,...
                                         lambda_I,S_Ilgn,rI_amb,...
                                         ...
                                         rhoE_ampa,rhoE_nmda,rhoI_ampa,rhoI_nmda,...
                                         gL_E,gL_I,Ve,Vi,LIFSimuT);
[mVI_dfI,~] = MEanFieldEst_SingleCell_4caSeq('i', f_EnI_dfI, SeediE, SeediI,...
                                         N_EE,N_EI,N_IE,N_II,...
                                         S_EE,S_EI,S_IE,S_II,p_EEFail,...
                                         lambda_E,S_Elgn,rE_amb,S_amb,...
                                         lambda_I,S_Ilgn,rI_amb,...
...
f_EnI0_dfI = MeanFieldEst_BkGd(N_EE,N_EI,N_IE,N_II,...
                                   S_EE,S_EI,S_IE,S_II,p_EEFail,...
                                   lambda_E,S_Elgn,rE_amb,S_amb,...
                                   lambda_I,S_Ilgn,rI_amb,...
                                   gL_E,gL_I,Ve,Vi,mVE_dfI,mVI_dfI);                               
%% Assemble Jacobian and determine iteration results
J_F = [(f_EnI0_dfE-f_EnI0)/dfE(1), (f_EnI0_dfI-f_EnI0)/dfI(2)];
J_Newton = (J_F-eye(2));

f_EnIRes = f_EnIIni - h_Step*J_Newton^-1*(f_EnI0-f_EnIIni);                             
f_EnIRes = max([f_EnIRes,[0;0]],[],2);                                  
%f_EnIIni = f_EnIRes*h_Step + f_EnIIni*(1-h_Step);

loop = loop+1;

f_EnIOut = [f_EnIOut,f_EnIRes];
meanVs = [meanVs, [mVE;mVI]];

%% Judge if steady
if ~SteadyIndicate
  if loop >= TestPoints && max(std(f_EnIOut(:,end-TestPoints+1:end),0,2) ...
                            ./mean(f_EnIOut(:,end-TestPoints+1:end),2))<0.025 % std/mean for the last 10 samples
     SteadyIndicate = true;
  end  
else 
...
function [meanV,fr] = MEanFieldEst_SingleCell_4caSeq(NeuronType, f_EnI, seedE, seedI,...
                                         N_EE,N_EI,N_IE,N_II,...
                                         S_EE,S_EI,S_IE,S_II,p_EEFail,...
                                         lambda_E,S_Elgn,rE_amb,S_amb,...
                                         lambda_I,S_Ilgn,rI_amb,...
