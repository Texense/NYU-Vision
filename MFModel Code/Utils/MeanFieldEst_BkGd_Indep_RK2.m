...
%% Runge-Kutta 2!
function [f_EnIOut,meanVs,loop,SteadyIndicate] = MeanFieldEst_BkGd_Indep_RK2(C_EE,C_EI,C_IE,C_II,... %4
...
[f1,~] = FrIter(f_In, ...
                ...
                gL_E,gL_I,Ve,Vi);
f_inter = f_In + h/2*(f1 - f_In);

[f2,mV2] = FrIter(f_inter, ...
                N_EE,N_EI,N_IE,N_II,...
                S_EE,S_EI,S_IE,S_II,p_EEFail,...
                lambda_E,S_Elgn,rE_amb,S_amb,...
                lambda_I,S_Ilgn,rI_amb,...
                ...
                rhoE_ampa,rhoE_nmda,rhoI_ampa,rhoI_nmda,...
                gL_E,gL_I,Ve,Vi);
f_Out = f_In + h*(f2 - f_inter);

loop = loop+1;

f_EnIOut = [f_EnIOut,f_Out];
meanVs = [meanVs, mV2];
...
%% One step iteration: constructing F
% Using input firing rate to determine output firing rate

function [f_OneStep,mV_OneStep] = FrIter(f_EnI0, ...
                                         N_EE,N_EI,N_IE,N_II,...
                                         S_EE,S_EI,S_IE,S_II,p_EEFail,...
                                         lambda_E,S_Elgn,rE_amb,S_amb,...
                                         lambda_I,S_Ilgn,rI_amb,...
